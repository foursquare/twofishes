...
Serving
=======
*   ./serve.py -p 8080 /output/dir – Where /output/dir will contain a subdirectory whose name will be the date of the most recent build (for example `2013-02-25-01-08-23.803740`. You need to point to this subdirectory or to a folder called `latest` which is created (in the twofishes directory) during the build process and is a symlink to the most recent dated subdirectory.
