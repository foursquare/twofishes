...
import com.foursquare.twofishes.indexer.common.GeonamesCountryInfo
...
import com.foursquare.twofishes.indexer.FeatureLoader
...
class GeonamesParser(
...
) extends DurationUtils with FeatureLoader with Logging {
  lazy val config = GeonamesImporterConfigParser.parse(args)

  lazy val slugIndexer = new SlugIndexer()
  def getSlugEntryMap = slugIndexer.slugEntryMap
  ...
  val adminIdMap = GeonamesCountryInfo.adminIdMap.clone()
  ...
  private def parseFromFile(filename: String,
    ...
    for {
      ...
      val recordsToInsert = lineGroup.zipWithIndex.flatMap({case (line, index) => {
        ...
        lineProcessor(realIndex, line).filter(f => shouldTakeFeature(f, allowBuildings)).map(inputFeature => {
          Stats.time("parse.line." + typeName) {
            parseFeature(inputFeature)
          }
        })
      }}).toList
  ...
  def isLocalLang(lang: String, cc: String) =
    GeonamesCountryInfo.countryLangMap.getOrElse(cc, Nil).contains(lang)
  ...
  def parseFeature(feature: InputFeature): GeocodeRecord = {
    ...
    val ids: List[StoredFeatureId] = List(geonameId) ++
      concordanceMap.get(geonameId).flatMap(concordanceId => {
        slugIndexer.slugEntryMap(concordanceId) = (SlugEntry(geonameId.humanReadableString, 0))
    ...
    if (feature.featureClass.woeType == YahooWoeType.COUNTRY) {
      GeonamesCountryInfo.countryNameMap.get(feature.countryCode).foreach(name =>
        displayNames ::=
    ...
    def fixParent(p: String): Option[String] = {
      GeonamesCountryInfo.adminIdMap.get(p) orElse {
