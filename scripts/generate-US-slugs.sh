...
if (List(YahooWoeType.TOWN, YahooWoeType.COUNTRY, YahooWoeType.ADMIN1, YahooWoeType.ADMIN2).contains(f.woeType)) {
  if ((f.population.getOrElse(0) > 0 || f.boost.getOrElse(0) > 0) && f.cc == \"US\") {
